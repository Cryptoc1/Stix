...
;
;	Roughly based on mikeOS's bootloader, as far as loading the kernel into memory
;	goes. Other than that, it's mostly my own :)
;
;		-Sam Steele, 12/2/2015
;
...
OEMLabel			db "STIXBOOT"	; Disk label
...
VolumeLabel			db "STIXOS     "; Volume Label: any 11 chars
...
start:
	...
	add ax, 288				; Define segment size or offset? (4096 + 512) / 16 = 288 bytes per paragraph
	mov ss, ax
	mov sp, 4096
...
read_fat_ok:
	...
	mov ax, 0x2000			; Segment where we'll load the kernel
	mov es, ax
	mov bx, 0
...
.done:
	...
	times 510-($-$$) db 0	; Pad remaining space to fulfill 512K requirement
	dw 0xAA55				; Identifies MBR (boot sector) on disk (required)
